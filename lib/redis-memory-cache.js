...
function RedisMemcacheClient(redisClient, redisSubClient, options, cb) {
     ...
     *   JSON-parsed?
     */
    var getCache = function(key, json) {
        if (json) {
            if (!jsonCache.hasOwnProperty(key)) {
                jsonCache[key] = getJson(cache[key]);
            }
            return _.cloneDeep(jsonCache[key]);
        }
        return cache[key];
    };
    ...
    this.get = function(key, cb, json) {
        ...
        if (this.pathIsEnabled(key.split(':'))) {
            ...
            if (key in cache) {
                // Return from cache.
                stats.hits++;
                return cb(null, getCache(key, json));
            } else {
     ...
     *   If true, returns as parsed json.
     */
    this.mget = function(keys, cb, json) {
        var values = {};
        var fromRedis = [];
            ...
            redisClient.mget(fromRedis, function(err, vals) {
                ...
                for (var i in fromRedis) {
                    if (self.pathIsEnabled(fromRedis[i].split(':'))) {
                        ...
                        values[fromRedis[i]] = getCache(fromRedis[i], json);
                    } else {
                        values[fromRedis[i]] = json ? getJson(vals[i]) : vals[i];
                    }
                }

                cb(null, _.values(values));
            });
        } else {
            cb(null, values);
        }
